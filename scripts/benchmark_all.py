...
def benchmark_model(
    *,
    ...
    Framework.setup(
        config_path=str(base_dir / "training_config.yaml"), require_custom_config=True
    )
    ...
    if len(dataset.test()) == 0 and len(dataset.train()) == 0:
        raise Framework.InferenceError(
            f"No images found for benchmarking in {base_dir}."
        )
    for _ in Logger.log_progress(
        range(num_warmup_iterations), leave=False, desc="Warming Up"
    ):
        for view in dataset:
    ...
    for _ in Logger.log_progress(
        range(num_iterations), leave=False, desc="Benchmarking Performance"
    ):
        for view in dataset:
            renderer.render_image(view, benchmark=True)
    torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        metavar="path/to/output/FasterGS",
        required=False,
